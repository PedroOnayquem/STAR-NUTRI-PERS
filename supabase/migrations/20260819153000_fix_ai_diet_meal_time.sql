-- Postgres does not implicitly cast a PL/pgSQL text expression to time.
-- Keep the complete diet write atomic while converting the model's HH:MM value.
begin;
...
declare
  v_diet_id uuid;
  v_meal record;
  v_meal_time time without time zone;
...
begin
  ...
  loop
    ...
    begin
      v_meal_time := nullif(btrim(v_meal.value ->> 'meal_time'), '')::time;
    exception
      when invalid_datetime_format or datetime_field_overflow then
        raise exception using errcode = '22023', message = 'Meal time must use a valid HH:MM value.';
    end;
